...
const Dashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      const [subscriptionRes, assignmentsRes, reportsRes] =
      ...
      if (reportsRes.status === "fulfilled") {
        ...
        const avgAi =
          completedReports.reduce(
            (sum, r) => sum + (r.ai_probability || 0),
            0
          ) / completedReports.length || 0;
        const avgPlagiarism =
          completedReports.reduce(
            (sum, r) => sum + (r.plagiarism_score || 0),
            0
          ) / completedReports.length || 0;
        ...
        setStats((prev) => ({
          ...
          pendingReports: reports.filter((r) => r.status === "processing")
            .length,
          avgAiScore: avgAi,
          ...
          <div className="flex items-center relative z-10">
            ...
            <div className="ml-4 flex-1">
              ...
              <p className="text-sm text-gray-600 mt-1">
                You have{" "}
                {stats.usageData?.remaining ??
                  Math.max(2 - (stats.totalReports || 0), 0)}{" "}
                free checks remaining out of 2 total. Upgrade to premium for
                monthly limits.
              </p>
              ...
              {(stats.usageData?.lifetimeUsage ?? stats.totalReports ?? 0) >=
                2 && (
                <div className="mt-2 text-xs text-red-600 font-medium">
                  ⚠️ Free limit reached. Upgrade to continue using the service.
                </div>
              )}
            </div>
          </div>
